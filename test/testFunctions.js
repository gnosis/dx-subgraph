...
const getContracts = async () => {
  ...
  [
    deployedContracts.DutchExchange,
    deployedContracts.TokenOWL,
    deployedContracts.TokenFRT
  ] = gasLogWrapper(proxiedContracts);
...
const checkBalanceBeforeClaim = async (
  ...
  const difference = balanceBeforeClaim
    .add(amt)
    .minus(balanceAfterClaim)
    .abs();
...
const claimSellerFunds = async (ST, BT, user, aucIdx, acct) => {
  ...
  let auctionIdx = aucIdx || (await getAuctionIndex(ST, BT));
  log('AUC IDX = ', auctionIdx);
  const [returned, tulipsIssued] = (await dx.claimSellerFunds.call(
    ST.address,
    BT.address,
    user,
    auctionIdx
...
const assertClaimingFundsCreatesMGNs = async (ST, BT, acc, type) => {
  const { DutchExchange: dx, TokenFRT: tokenMGN } = await getContracts();
  ...
  if (type === 'seller') {
    [, tulipsIssued] = (await dx.claimSellerFunds.call(
      ST.address,
      BT.address,
      acc,
      auctionIdx - 1
    ...
    [, tulipsIssued] = (await dx.claimBuyerFunds.call(
      ST.address,
      BT.address,
      acc,
      auctionIdx - 1
...
const checkUserReceivesTulipTokens = async (ST, BT, user, idx, lastClosingPrice) => {
  ...
  const [returned, tulips] = (await dx.claimBuyerFunds.call(
    ST.address,
    BT.address,
    user,
    aucIdx
  )).map(amt => amt.toNumber());
  ...
  const {
    receipt: { logs }
  } = await claimBuyerFunds(ST, BT, user, aucIdx);
  ...
  if (STName === 'Ether Token' || BTName === 'Ether Token') {
    assert.isAtLeast(
      tulips.toEth(),
      returned.toEth(),
      'Auction closed returned tokens should equal tulips minted'
    );
  } else {
    log(`
  ...
  `);
  assert.equal(
    (tulips + lockedTulFunds).toEth(),
    lastestLockedTulFunds.toEth(),
    'for any Token pair, auction has cleared so returned tokens should equal tulips minted'
  );
};
...
const assertReturnedPlusMGNs = async (ST, BT, acc, type, idx = 1, eth) => {
  ...
  const [{ DutchExchange: dx }, STName, BTName] = await Promise.all([
    getContracts(),
    ST.name.call(),
    BT.name.call()
  ]);
  ...
  const [num, den] = (await dx.closingPrices.call(ST.address, BT.address, idx)).map(s =>
    s.toNumber()
  );
  const [hNum, hDen] = (await dx.getPriceInPastAuction.call(
    type === 'seller' ? ST.address : BT.address,
    eth.address,
    idx - 1
  )).map(s => s.toNumber());
  ...
  if (type === 'seller') {
    ...
    [returned, tulipsIssued] = (await dx.claimSellerFunds.call(
      ST.address,
      BT.address,
      acc,
      idx
    ...
    [returned, tulipsIssued] = (await dx.claimBuyerFunds.call(
      ST.address,
      BT.address,
      acc,
      idx
...
const unlockMGNTokens = async (user, ST, BT) => {
   ...
   */
  let [unlockedFunds, withdrawTime] = (await tokenMGN.unlockedTokens.call(user)).map(n =>
    n.toNumber()
  );
  log(`
  ...
  const totalAmtLocked = (await tokenMGN.lockTokens.call(lockedUserMGNs, {
    from: user
  })).toNumber();
  log(`
  ...
  if (aucIdx === 2) {
    ...
    assert.equal(
      totalAmtLocked,
      lockedUserMGNs,
      'Total locked tulips should equal total user balance of tulips'
    );
  } else {
  ...
  [unlockedFunds, withdrawTime] = (await tokenMGN.unlockTokens.call({ from: user })).map(t =>
    t.toNumber()
  );
  log(`
  AMT OF UNLOCKED FUNDS  = ${unlockedFunds.toEth()}
  ...
  if (aucIdx === 2) {
    ...
    assert.equal(
      withdrawTime,
      timestamp() + 24 * 3600,
      'Withdraw time should be equal to [(24 hours in seconds) + (current Block timestamp in seconds)]'
    );
  } else {
  ...
  for (let token of Tokens) {
    ...
    for (let tokenPartner of Tokens) {
      if (token.address !== tokenPartner.address) {
        let lastAuctionIndex = (await dx.getAuctionIndex.call(
          token.address,
          tokenPartner.address
        )).toNumber();
        // check old auctions balances
        for (let auctionIndex = 1; auctionIndex < lastAuctionIndex; auctionIndex += 1) {
          for (let acct of Accounts) {
            if (
              (await dx.buyerBalances.call(
                token.address,
                tokenPartner.address,
                auctionIndex,
                acct
              ...
              const [w] = await dx.claimBuyerFunds.call(
                token.address,
                tokenPartner.address,
                acct,
                auctionIndex
            ...
            if (
              ...
              const [w] = await dx.claimSellerFunds.call(
                tokenPartner.address,
                token.address,
                acct,
                auctionIndex
...
const getClearingTime = async (sellToken, buyToken, auctionIndex) => {
  const { DutchExchange: dx } = await getContracts();
  return (await dx.getClearingTime.call(
